...
class TwitterOauth
  ...
  def initialize( user_token = nil, user_secret = nil )
    Rails.logger.info("########initialize"+TWOAUTH_SITE)
    Rails.logger.info("########TWOAUTH_SECRET"+TWOAUTH_SECRET)
    Rails.logger.info("########TWOAUTH_KEY"+TWOAUTH_KEY)
  ...
  def exchange_request_for_access_token( request_token,  request_token_secret, oauth_verifier )
    ...
    Rails.logger.info("########exchange_request_for_access_token######"+oauth_verifier.inspect)
  ...
  def verify_credentials
    Rails.logger.info("########verify_credentials")
    response = self.access_token.get('/account/verify_credentials.json')
    Rails.logger.info("########verify_credentials_response"+response.inspect)
    case response
      when Net::HTTPSuccess
        credentials=JSON.parse(response.body)
        raise TwitterOauth::UnexpectedResponse unless credentials.is_a? Hash
        Rails.logger.info("########verify_credentials_response"+credentials.inspect)
        credentials
      else
  ...
  def friends_by_page( user=nil, page = 1 )
    Rails.logger.info("########friends_by_page")
